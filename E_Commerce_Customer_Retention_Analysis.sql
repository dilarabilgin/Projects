...
CREATE VIEW CustOrderProducts
AS
SELECT DISTINCT	Cust_id, Prod_id,
		SUM(Order_Quantity) OVER (PARTITION BY Cust_id, Prod_id) Order_Product_Quantity,
...
--- tries
CREATE VIEW Cust_By_Month
AS
SELECT DISTINCT	Ord_Year, Ord_Month, Montly_Cust, Cust_id,
				LEAD(Montly_Cust) OVER(ORDER BY Ord_Year, Ord_Month) Next_Month_Cust
FROM	
		(
		SELECT DISTINCT Cust_id, YEAR(O.Order_Date) Ord_Year, MONTH(O.Order_Date) Ord_Month,
					COUNT(Cust_id) OVER(PARTITION BY YEAR(O.Order_Date), MONTH(O.Order_Date)) Montly_Cust
		FROM		market_fact M, orders_dimen O
		WHERE		M.Ord_id = O.Ord_id
		) A


SELECT DISTINCT	Ord_Year, Ord_Month, Montly_Cust, Next_Month_Cust,
				LAST_VALUE(Next_Month_Cust) OVER(PARTITION BY Ord_Year, Ord_Month ORDER BY Ord_Year, Ord_Month )
FROM			Cust_By_Month
ORDER BY		1, 2
